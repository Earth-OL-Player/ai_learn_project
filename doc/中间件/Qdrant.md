...
Qdrant 是启用 RAG 入库和向量检索时必选的向量数据库，用于保存学习资料、题目、标准答案、解析和知识点说明的向量索引，支撑 RAG 检索、智能出题、答案评分参考和学习建议生成。

边界说明：

- Qdrant 只保存向量、文本片段和必要 metadata。
- 用户、系统题库、刷题汇总等业务主数据仍保存在 MySQL，后端通过 `ai-service` 内部接口间接触发 RAG 任务。

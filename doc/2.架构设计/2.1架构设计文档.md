...
- 支撑学习路线展示、建议评论、热门面经、系统题库管理、AI智能刷题和个人成长体系。
- 建立用户、权限、题库、答题记录和成长体系的数据边界。
- 支撑 LangChain Agent 评分/讨论、系统题库管理和本地兜底策略；未接入主业务的 RAG/Qdrant 预留实现已移除。
...
flowchart TB
    ...
    subgraph app["应用服务"]
        ...
        ai["ai-service<br/>FastAPI<br/>Agent / 评分 / 讨论"]
    end

    subgraph middleware["中间件"]
        mysql[("MySQL<br/>业务主库")]
        redis[("Redis<br/>缓存 / 限流预留")]
    end

    user -->|访问页面| web
    web -->|REST API| backend
    backend -->|业务数据| mysql
    backend -.->|后续缓存、限流| redis
    backend -->|内部 API| ai
```
...
- AI 服务当前处理 Agent 编排、评分和本题讨论，不直接对浏览器开放；未接入主业务的 RAG 入库/检索预留接口已删除。
- MySQL 保存业务主数据；Redis 当前仅文档预留，尚未接入运行代码；Qdrant 相关预留代码和配置已移除。
...
| 类型 | 选型 | 说明 |
| --- | --- | --- |
| 运行时 | Node.js 20 LTS 或 22 LTS | 具体 patch 版本由 `.nvmrc`、Volta 或锁文件固定 |
...
| RAG 索引 | LlamaIndex 或等价方案 | 后续文档切分、索引和检索管理预留；当前主业务未启用 |
...
- `rag`：后续文档切分、向量化、检索、重排序预留模块；当前主业务不调用。
...
后端鉴权、读取题库、读取答题记录、组装上下文
  ↓
AI 服务进行评分/讨论；后续可增强为画像分析、推荐或生成题目
  ↓
...
| 候选题召回 | 知识点、难度、标签 | 候选题和知识片段 | 当前由 MySQL 题库和答题统计加权抽题；后续可从题库和向量库召回内容 |
...
```text
题目权重 = 基础权重 + 未刷加权 + 低分加权 + 重要性评分加权 + 真实面试出现次数轻量加成 - 高频重复惩罚
```
...
| `suggestions` | 用户建议 |
| `comments` | 评论和回复 |
| `questions` | 系统题库，按题目编码维护稳定业务标识 |
| `user_question_stats` | 用户与题目编码维度的轻量刷题汇总 |
| `user_practice_sessions` | 用户当前刷题阶段和当前题目编码 |
| `badges` | 徽章定义 |
| `user_badges` | 用户已获得徽章 |
...
- `questions` 只保留系统题库核心字段：`code`、`question`、`question_type`、`standard_answer`、`importance_score`、`occurrence_count`。
- 用户刷题数据只保存 `user_question_stats` 汇总，不保存完整答题记录和聊天明细。
...
1. 启动 MySQL。Redis 当前为预留能力，默认不需要启动。
2. 启动 `ai-service`；如只使用 Java 后端本地兜底评分，也可关闭 `AI_SERVICE_ENABLED`。
3. 使用 `local` 或 `dev` Profile 启动 `ai-learn-backend`。
...
| `REDIS_URL` | `redis://127.0.0.1:6379` | 后续后端连接 Redis 预留，当前非必填 |
...
- AI 服务仅允许后端调用。
- 当前生产部署默认只要求 MySQL；Redis 在正式接入对应功能前无需部署。
- 后续启用 Redis 后，初期可同机部署，数据增长后迁移到云 Redis 或托管服务。
- 生产配置通过环境变量或服务器私有配置注入，不进入仓库。
- MySQL 必须持久化并制定备份策略；Redis 是否持久化按使用场景决定。
...
| 第三阶段 | AI 服务增强 | FastAPI、AI 评分/讨论、题目推荐和 AI 出题作为后续增强 |
...
- AI 服务：`ai-service`，当前使用 FastAPI + LangChain/LangGraph 相关能力提供评分和讨论。
- 中间件：MySQL 作为当前必需业务主库；Redis 作为缓存和限流基础设施预留。

该方案在 MVP 阶段保持工程数量少、职责边界清楚，同时为智能刷题、成长体系和社区功能留下扩展空间。当前为了降低本地部署和服务器运维成本，未接入主业务的 RAG/Qdrant 预留实现已移除。

...
- 支撑学习路线展示、建议评论、面试题占位和刷题 Agent 基础流程。
...
- `layout`：整体布局、侧边栏、顶部用户区。
- `router`：路由配置和登录守卫。
- `pages/learning-roadmap`：AI 学习路线与资料。
- `pages/suggestions-comments`：建议与评论。
- `pages/interview-questions`：面试题占位页。
- `pages/practice-agent`：刷题 Agent 页面。
- `pages/profile`：个人中心。
- `api`：后端接口封装。
- `stores`：登录态和全局状态。
...
└── agent/                     # 刷题 Agent 上下文：刷题会话、AI 服务调用、结果校验和保存
```
...
| Agent 编排 | LangGraph | 管理刷题 Agent 工作流 |
...
| 第五阶段 | 内容和社区增强 | 面试题大全、评论互动、收藏、后台管理、学习统计 |

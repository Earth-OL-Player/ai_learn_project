...
文件：`V1__init_user_tables.sql` 初始化用户表；`V2__require_nickname_email.sql` 将昵称和邮箱调整为必填，并为昵称增加唯一索引。
...
1. 注册：校验参数和用户名、昵称、邮箱唯一性，生成密码哈希，创建用户，签发 JWT。
...
- 默认过期时间：7200 秒。
- 登录用户调用后端接口且携带的 token 校验通过后，后端通过响应头 `X-Refresh-Token` 返回新 token，前端读取该响应头并覆盖 `ai_learn_access_token`，实现活跃自动续期。
- CORS 需要暴露 `X-Refresh-Token` 响应头，确保浏览器前端可以读取。
- `JWT_SECRET` 不得写死在代码中，统一从 `ai-learn-backend/.env` 本地配置读取。
...
后端本地敏感配置统一写入 `ai-learn-backend/.env`，Spring Boot 启动时读取后再映射到 `application.yml`。

```env
DATABASE_URL="jdbc:mysql://127.0.0.1:3306/ai_learn?useUnicode=true&characterEncoding=utf8&serverTimezone=Asia/Shanghai"
DATABASE_USERNAME="用户名占位符"
DATABASE_PASSWORD="密码占位符"
JWT_SECRET="JWT_SECRET占位符"
JWT_EXPIRES_IN_SECONDS="7200"
```

`application.yml` 通过占位符读取上述配置：
...
- 验证注册成功流程。
- 验证重复用户名、重复昵称、重复邮箱会返回明确错误。
- 验证登录成功和密码错误两类流程。
- `/users/me` 未登录返回 `AUTH_UNAUTHORIZED`。
- `/users/me` 登录后不返回密码哈希。
- 前端手工验证刷新恢复登录态。
- 前端手工验证访问后端接口且携带有效 token 后，响应头 `X-Refresh-Token` 会刷新本地 token。

...
系统 SHALL 将前端根路径 `/` 重定向到 `/learning-roadmap`，并默认展示“AI 学习路线和资料”页面。

#### Scenario: 访问根路径
- **WHEN** 用户在浏览器访问前端根路径 `/`
- **THEN** 系统 SHALL 自动进入 `/learning-roadmap` 并高亮“AI 学习路线和资料”菜单

### Requirement: 全局布局展示规划菜单
系统 SHALL 提供清新简约的全局布局，包含左侧菜单、顶部用户区和主内容区；左侧菜单 SHALL 展示“AI 学习路线和资料”“建议评论区”“面试题大全”“刷题 Agent”四个入口。
...
系统 SHALL 为“建议评论区”“面试题大全”“刷题 Agent”提供占位页面，页面包含明确标题和后续迭代说明，不得出现接口报错或空白页。

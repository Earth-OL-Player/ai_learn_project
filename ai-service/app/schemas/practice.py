...
class PracticeGradeResponse(BaseModel):
    ...
    problems: list[str] = Field(default_factory=list, description="用户答案存在的表达、逻辑或概念问题，使用简短中文短句。")
    referenceAnswer: str = Field(description="参考答案原文。")
    improvementAdvice: str = Field(description="基于 missingPoints 和 problems 给出最优先的改进建议；要求具体、可操作，不要空泛鼓励；一到两句话。")
